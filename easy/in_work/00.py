...
class Room:
    ...
    def reserve(self, start_date: date, end_date: date) -> bool:
        ...
        if self.is_available(start_date, end_date):
            self.reservation_periods.append([start_date, end_date])
            return True
        return False

    def cancel_reservation(self, start_date: date, end_date: date) -> bool:
        """Отменить бронирование на указанный период."""
        for i in range(len(self.reservation_periods)):
            if self.reservation_periods[i][0] == start_date and self.reservation_periods[i][1] == end_date:
                del self.reservation_periods[i]
                return True
        return False
