...
df.columns = ['Серия', 'Эмитент', 'Дата размещения', 'Дата погашения',
              'Объем выпуска, млрд руб.', 'Ставка купона, %']

df['Ставка купона, %'] = df['Ставка купона, %'].map(lambda x: x[:-1].replace(",", ".")).astype(float)
df['Объем выпуска, млрд руб.'] = df['Объем выпуска, млрд руб.'].map(lambda x: x.split()[0].replace(",", ".")).astype(float)

print(df['Серия','Дата погашения'])